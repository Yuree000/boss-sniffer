...
(function (global) {
  ...
  /**
   * 主入口:把 JD 模板拼成完整 SYSTEM_PROMPT 字符串(三段式 / 喂给 LLM)
   * 拼装顺序:角色行 → 岗位规则 → 信息源优先级 → 决策规则 → 输出 schema
   *
   * 参数:
   *   - jd: BossJD 模板对象,必须含 mustConditions / optionalConditions / optionalThreshold
   *
   * 返回: 完整 system prompt 字符串(可直接传给 LLM client)
   */
  function build(jd) {
  ...
  // === 段 2：岗位规则（动态）===
  /**
   * 拼"岗位规则"段:必要条件用 M1/M2/... 列表,可选条件用 O1/O2/... 列表 + 阈值 K
   *
   * 参数:
   *   - must: 必要条件数组(每条含 text 字段)
   *   - opt: 可选条件数组
   *   - K: 可选满足阈值
   *
   * 返回: 多行字符串(带 # 段标题)
  ...
  /**
   * 拼"信息源优先级"段(静态文本):告诉 LLM chat > domDetail > 简历字段 > bossSignals 的取舍规则
   *
   * 返回: 多行字符串(整段静态,不依赖 JD)
   */
  ...
  /**
   * 拼"决策规则"段:含必要/可选集合数学描述 + 三态判定 + "不能反推"铁律 + 反面示例
   * 是 prompt 里最长的一段,逻辑约束 LLM 输出严格"符合"or"pass"
   *
   * 参数:
   *   - must / opt: 必要 / 可选条件数组
   *   - K: 可选满足阈值
  ...
  /**
   * 拼"输出 schema"段:动态生成 mustBreakdown / optionalBreakdown 的 JSON 模板行
   * 严格约束 LLM 输出"只输出纯 JSON 对象"+ 字段顺序 + reason 写法
   *
   * 参数:
   *   - must / opt: 必要 / 可选条件数组(决定生成 M1..Mn / O1..On 几个键)
   *
   * 返回: 多行字符串(含 JSON 模板花括号示例)
   */
