...
(function (global) {
  ...
  // 把 callback 风格的 chrome API 包成 Promise
  // 自愈：遇到"Another debugger already attached" → 试 detach 再重 attach 一次
  // 这能解决"我们自己上一次没干净 detach"的情况。
  // 不能解决：用户开着 DevTools / 别的扩展占着 — 那种要清晰报错让用户介入
  function attachDebuggerOnce(tabId) {
    return new Promise(function (resolve, reject) {
      chrome.debugger.attach({ tabId: tabId }, '1.3', function () {
        if (chrome.runtime.lastError) {
          reject(err('DebuggerAttachError', chrome.runtime.lastError.message));
        } else { resolve(); }
      });
    });
  }
  async function attachDebugger(tabId) {
    try {
      ...
      // 自愈：如果是 "Another debugger is already attached"，先 detach 再重试一次
      if (/already attached|Another debugger/i.test(msg)) {
        ...
        try {
          ...
          throw err('DebuggerAttachError',
            e2.message + '（自愈失败 — 请关闭 BOSS 页 DevTools / 禁用其他用 debugger 的扩展（如智聘 AI），或刷新 BOSS tab 后重试）');
        }
      }
      throw e;
    }
  ...
  function detachDebugger(tabId) {
    return new Promise(function (resolve) {
      chrome.debugger.detach({ tabId: tabId }, function () {
        // 即使报错也 resolve（detach 失败通常是已经 detach 过）
        resolve();
      });
    });
  }
  ...
  // 单次 sayHi 流程：attach → 找按钮 → 滚动到可视 → 再找一次 → 点击 → detach
  ...
  // 把 sayHi 状态写回评估记录（让 sidepanel 能展示）
  // 由 background.js 提供 BOSS_EVAL_GREETING_PATCHER（参数：candidateId, greetingPatch）
