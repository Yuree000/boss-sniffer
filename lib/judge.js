...
(function (global) {
  ...
  /**
   * 把候选人对象序列化为对 LLM 友好的紧凑 markdown 文本(剥噪/去 null/保字段路径)
   * 这是喂给 LLM 的 user prompt 主体,字段越精简 LLM 越聚焦
   *
   * 参数:
   *   - c: 标准候选人对象(extractor 输出 + chatHistory + domDetail)
   *
   * 返回: 多行字符串(markdown 风格,带 ## 小节)
   */
  ...
  const sleep = self.BossRuntimeUtils.sleep; // v1.1.22 提到 lib/runtime-utils.js

  /**
   * 主入口:异步评估单个候选人(带重试)
   * 流程:序列化候选人 → 调 LLM → 解析 JSON → 校验 schema → 失败按规则重试
   *
   * 参数:
   *   - candidate: 标准候选人对象(必须含 basic 字段)
   *   - jd: BossJD 模板 { jdId, name, mustConditions[], optionalConditions[], customPrompt, ... }
   *   - llmConfig: LLM 配置 { protocol, baseUrl, apiKey, model, ... } 需含 apiKey/model
   *
   * 返回: 评估结果 { decision, mustBreakdown, optionalBreakdown, reason, attempts, perAttempt, ... }
   * 失败抛错(错误对象上挂 perAttempt 重试轨迹给上游写入 evaluation)
   */
  async function judgeCandidate(candidate, jd, llmConfig) {
    ...
    // v0.25.2：HR 自定义 prompt 优先（jd.customPrompt 非空时直接用，跳过 prompt-builder）
    //   null / 空字符串 / 全空白 → 走 prompt-builder.build(jd) 自动生成
    //   HR 通过 admin 预览 prompt modal 的「修改」+「保存」写入 jd.customPrompt
    //   「恢复默认」清空 jd.customPrompt
    const customPrompt = jd.customPrompt;
    const systemPrompt = (typeof customPrompt === 'string' && customPrompt.trim())
      ? customPrompt
      : self.BossPromptBuilder.build(jd);
