...
(function (global) {
  ...
  /**
   * 把 "2020.06-2022.08" / "2018至2022" 这种时间区间描述拆成起止两段
   *
   * 参数:
   *   - timeDesc: 字符串,BOSS 接口返回的工作/教育经历时间描述
   *
   * 返回: { from, to } 两个字符串(没解析到就 null)
   */
   ...
   * BOSS 接口里高亮词有时是字符串、有时是 {content,...} 对象,这里统一拍成字符串数组
   *
   * 参数:
   *   - arr: BOSS 接口里的高亮词数组(混合类型)
   *
   * 返回: 字符串数组(空就 null,方便 JSON 序列化时省字段)
   */
  ...
  // ========== 场景 1：chat/geek/info ==========
  /**
   * 场景 1：HR 点开候选人聊天窗口时,BOSS 后台返回的详细资料接口
   * 归一化成统一的候选人对象 schema(见 schemas/candidate.js)
   *
   * 参数:
   *   - apiResponse: BOSS /wapi/zpjob/chat/geek/info 的原始 JSON
   *
   * 返回: 标准候选人对象;数据不全(没有 uid)就返回 null
   */
  ...
  /**
   * 场景 2:从 BOSS 推荐流(rec/geek/list)一条 item 抽出一个候选人对象
   * 比场景 1 少"现居城市/活跃状态"等字段,但多了"工作描述/markWords/期望地"
   *
   * 参数:
   *   - item: BOSS 推荐列表里单条候选人结构(geekCard + showWorks/showEdus)
   *   - batchAt: 同一批 15 个候选人共享的时间戳(保证整批一起排序)
   *   - indexInBatch: 候选人在 Boss 推荐流里的视觉位置(0 = 页面最顶部)
   *
   * 返回: 标准候选人对象;item 不合法返回 null
   */
  ...
  /**
   * 场景 2 入口:批量解析一整页推荐流(15 人左右),返回候选人数组
   *
   * 参数:
   *   - apiResponse: BOSS /wapi/zpjob/rec/geek/list 原始 JSON
   *
   * 返回: 候选人对象数组(可能为空数组)
   */
  ...
  /**
   * 场景 2b:从"最新"tab 列表(bossGetGeek)抽一个候选人,字段映射与 recList 略有差异
   *
   * 参数:
   *   - item: bossGetGeek 列表里单条候选人结构
   *   - batchAt: 同批共享时间戳(整批排序用)
   *   - indexInBatch: 在列表里的位置序号
   *
   * 返回: 标准候选人对象;item 不合法返回 null
   */
  ...
  /**
   * 场景 2b 入口:批量解析"最新"tab 一整页,返回候选人数组
   *
   * 参数:
   *   - apiResponse: BOSS /wapi/zprelation/interaction/bossGetGeek 原始 JSON
   *
   * 返回: 候选人对象数组(可能为空)
   */
  ...
  /**
   * 从沟通页 .geek-item 卡片的可见文本里清洗出"候选人主动招呼内容"
   * 把时间戳/未读数/候选人名/岗位名等前缀都剥掉,只留招呼正文
   *
   * 参数:
   *   - visibleText: 卡片整段可见文本
   *   - name: 候选人姓名(作为剥离锚点)
   *   - jobName: 我方岗位名(作为剥离锚点)
   *
   * 返回: 招呼正文字符串;太短(<=2 字)就 null
   */
  ...
  /**
   * 场景 3:从沟通页 .geek-item Vue 反射出来的 props 里抽出候选人对象
   * 字段比接口少很多(age/city 等都是 null),靠 chat/geek/info 后续补全
   *
   * 参数:
   *   - item: content.js DOM 扫描+ Vue 反射结果(uid/name/jobName/visibleText 等)
   *
   * 返回: 标准候选人对象;没有 uid 返回 null
   */
  ...
  /**
   * 场景 3 入口:批量处理沟通页扫到的 N 张 .geek-item 卡片,返回候选人数组
   *
   * 参数:
   *   - items: content.js 返回的 DOM 卡片数组(每条含 Vue props)
   *
   * 返回: 候选人对象数组(每条 source.indexInBatch 按入参顺序赋值)
   */
  ...
  /**
   * 把 BOSS 一条聊天消息的 body 归一化成 { text, kind } 两字段
   * 文本/简历卡片/链接卡/交互卡都统一拍成可读文本
   *
   * 参数:
   *   - msg: BOSS 原始消息对象(含 body.text / body.resume / body.hyperLink / body.action)
   *
   * 返回: { text: 可读文本, kind: 'text'|'resume'|'hyperLink'|'action'|'unknown' }
   */
  ...
  /**
   * 场景 4 附属:从 historyMsg 里挑出"BOSS 推的简历卡片消息"(type=3,bizType=21050004)
   * 把简历卡里的 applyStatus / content1/2/3 / bottomText 等做成结构化字段
   *
   * 参数:
   *   - rawMessages: BOSS historyMsg.messages 原始消息数组
   *
   * 返回: 简历卡片数组 [{ candidateId, applyStatus, name, content1, ... }]
   */
  ...
  /**
   * 场景 4 入口:解析一段聊天历史消息接口,返回会话级聚合信息
   * 注意:这个接口返回的是"消息列表",不是"一个人",所以返回 uids 候选列表
   * background.js 拿 uids 去 evaluations/sayhi_pool 反查,确定谁是候选人
   *
   * 参数:
   *   - apiResponse: BOSS /wapi/zpchat/boss/historyMsg 原始 JSON
   *
   * 返回: { uids: 所有出现过的 uid, messages: 归一化消息, lastMessageAt, resumeCards }
   */
  ...
  /**
   * 抽取统一入口:按 apiPath 路由到对应抽取函数,返回标准化数组
   * 让下游 background.js 用同一个循环处理所有场景(场景 1 = 单元素数组)
   *
   * 参数:
   *   - apiPath: BOSS 接口路径(用来识别走哪个抽取分支)
   *   - apiResponse: 对应接口的原始 JSON
   *
   * 返回: 候选人对象数组(无法识别就返回空数组)
   */
  function extractFromCapture(apiPath, apiResponse) {
    ...
    // 不做：view/geek/info/v2 字段加密（POC A5 真机验证为死路：18KB 加密块 + WASM，主线放弃）
    return [];
  }
  ...
  /**
   * 解析候选人详情面板里的"期望/最近关注"那一行
   * 例: "期望:成都·前端开发 15-25K" → { prefix, cityRaw, cities, jobRaw, salaryRaw }
   * 多城市用 & / 顿号分隔
   *
   * 参数:
   *   - rawText: DOM 里抓到的原始期望文本(含前缀)
   *
   * 返回: 结构化对象 { prefix, original, parts, cityRaw, cities, jobRaw, salaryRaw } 或 null
   */
  ...
  /**
   * 场景 5:把 inject.js 扫详情面板 DOM 得到的纯对象,归一化成 domDetail 结构
   * 挂到 candidate.bossSignals.domDetail 给 LLM 用(主要补 desc/workExp.description)
   *
   * 参数:
   *   - rawScan: inject.js 详情面板 DOM 扫描结果(纯对象,无 DOM 引用)
   *
   * 返回: domDetail 标准结构(失败返回 null)
   */
